...
<template>
  <main class="flex min-h-screen items-center justify-center bg-slate-950 p-6 text-slate-200">
    <button class="fixed right-5 top-5 rounded-xl border border-emerald-400/40 bg-slate-900/80 px-3 py-2 font-mono text-xs text-emerald-200" @click="toggleLocale()">{{ locale === 'zh' ? 'EN' : '中文' }}</button>
    <section class="w-full max-w-md rounded-[32px] border border-emerald-500/25 bg-slate-900/75 p-8 shadow-[0_0_80px_rgba(16,185,129,.14)] backdrop-blur-xl">
      <div class="mb-8 text-center">
        ...
        <h1 class="mt-3 text-3xl font-black text-white">{{ t('accessConsole') }}</h1>
      </div>
      <form class="space-y-4" @submit.prevent="submit">
        <input v-model="username" class="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 font-mono outline-none transition focus:border-emerald-400" :placeholder="t('username')" />
        <input v-model="password" type="password" class="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 font-mono outline-none transition focus:border-emerald-400" :placeholder="t('password')" />
